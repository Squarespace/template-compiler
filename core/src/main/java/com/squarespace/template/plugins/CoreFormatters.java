...
public class CoreFormatters implements FormatterRegistry {
  ...
  public static class ApplyFormatter extends BaseFormatter {
    ...
    public void apply(final Context ctx, Arguments args, Variables variables) throws CodeExecuteException {
      ...
      // The scoped run below captures these in a lambda, so they are
      // copied into final locals first.
      final Instruction template = inst;
      final boolean privateCtx = privateContext;
      final ObjectNode partialArgs = argvar;
      if (!ctx.partialScoped(name,
          () -> var.set(executeTemplate(ctx, template, var.node(), privateCtx, partialArgs)))) {
        var.setMissing();
      }
    }
  ...
  public static class EncodeUriFormatter extends BaseFormatter {
    ...
    public void apply(Context ctx, Arguments args, Variables variables) throws CodeExecuteException {
      ...
      String encoded = EncodeUtils.encodeURI(value, ctx.compatEnabled(Patch.ENCODE_URI_QUOTE));
  ...
  public static class EncodeUriComponentFormatter extends BaseFormatter {
    ...
    public void apply(Context ctx, Arguments args, Variables variables) throws CodeExecuteException {
      Variable var = variables.first();
      String value = var.node().asText();
      String encoded = EncodeUtils.encodeURIComponent(value, ctx.compatEnabled(Patch.ENCODE_URI_QUOTE));
      // EncodeUtils returns null on lone surrogates. Fixed, that emits
      // empty output instead of the text "null".
      if (encoded == null && !ctx.compatEnabled(Patch.ENCODE_URI_SURROGATE)) {
        encoded = "";
